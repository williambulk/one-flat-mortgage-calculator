document.addEventListener("DOMContentLoaded", function () {
  if (!document.querySelectorAll(".off-mortgage-calculator").length) return;

  const offMortgage = document.querySelectorAll(
    ".off-mortgage-calculator"
  );

  offMortgage.forEach((calculator) => {
    ...
    const calculateMortgage = () => {
      ...
      document.querySelector(".off-mortgage-estimate").innerHTML = result;
    };
  });
});
